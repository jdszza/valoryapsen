"""
APSEN - Dashboard Executivo (porta 8050)
Informativo e visual — para gestores. Requer login.
"""
import json
import os
import threading
...
def api(method, path, token=None, **kwargs):
    ...
    try:
        r = getattr(requests, method)(
            f"{BACKEND_HTTP}{path}", headers=headers, timeout=5, **kwargs
        )
...
STATUS_LABEL = {"idle": "AGUARDANDO", "running": "EM PRODUÇÃO",
                "paused": "PAUSADO",  "alarm":   "ALARME"}
        ...
        html.Div(style={
            ...
            html.Div([
                html.H1("APSEN", style={"color": AZUL, "fontWeight": "900",
                                         "letterSpacing": "4px", "fontSize": "36px",
                                         "margin": "0"}),
                html.P("Dashboard Executivo", style={"color": CINZA, "fontSize": "14px",
                                                      "margin": "4px 0 32px"}),
            ], className="text-center"),
            html.Label("Usuário", style={"fontWeight": "600", "fontSize": "13px",
                                          "color": "#555"}),
            dbc.Input(id="login-user", type="text", size="lg", className="mb-3"),
            html.Label("Senha", style={"fontWeight": "600", "fontSize": "13px",
                                        "color": "#555"}),
            dbc.Input(id="login-senha", type="password", size="lg", className="mb-4"),
            dbc.Button("ENTRAR", id="btn-login", size="lg", className="w-100",
                       style={"background": AZUL, "border": "none",
                              "fontWeight": "700", "letterSpacing": "2px"}),
            html.Div(erro, style={"color": VERM, "textAlign": "center",
...
def kpi(titulo, valor, sub="", cor=AZUL):
    return html.Div(style={
        ...
        html.Div(titulo, style={"fontSize": "12px", "color": "#999",
                                 "textTransform": "uppercase", "letterSpacing": "1px",
                                 "fontWeight": "600"}),
        html.Div(valor, style={"fontSize": "38px", "fontWeight": "900",
                                "color": "#1a1a2e", "lineHeight": "1.1",
                                "margin": "4px 0"}),
        html.Div(sub, style={"fontSize": "12px", "color": "#aaa"}),
    ])
...
def pagina_dash(auth):
    ...
    return html.Div(style={"background": "#f0f4f8", "minHeight": "100vh",
        ...
        html.Div(style={
            ...
            html.Div([
                html.Span("APSEN", style={"fontSize": "22px", "fontWeight": "900",
                                           "letterSpacing": "3px"}),
                html.Span("  Dashboard Executivo",
                          style={"fontSize": "13px", "opacity": ".7", "marginLeft": "8px"}),
            ]),
            html.Div([
                html.Span(f"{nome} · {role.upper()}",
                          style={"fontSize": "13px", "opacity": ".8", "marginRight": "16px"}),
        ...
        html.Div(style={"padding": "28px 32px"}, children=[
            ...
            html.Div(style={"display": "flex", "gap": "20px", "flexWrap": "wrap"}, children=[
                # Gráfico principal
                html.Div(style={
                    "flex": "3", "minWidth": "320px", "background": "#fff",
                    "borderRadius": "12px", "padding": "24px",
                    "boxShadow": "0 2px 8px rgba(0,0,0,.06)",
                }, children=[
                    html.H3("Produção ao Longo do Tempo",
                    ...
                    html.Div(style={
                        ...
                        html.H3("Progresso do Lote",
                                style={"margin": "0 0 12px", "fontSize": "15px",
                                       "color": AZUL, "fontWeight": "700"}),
                        dcc.Graph(id="gauge-prog", style={"height": "200px"},
                                  config={"displayModeBar": False}),
                    ]),
...
@callback(
    ...
    Output("alarme-banner", "children"),
    Output("graph-prod", "figure"),
    Output("gauge-prog", "figure"),
...
def atualizar(_, auth):
    ...
    kpis = [
        kpi("Contagem Atual", f"{contagem:,}", f"Lote: {lote_id}", AZUL),
        kpi("Meta",           f"{meta:,}",     "unidades",          CINZA),
        kpi("Progresso",      f"{prog}%",       f"{contagem:,} / {meta:,}",
            VERDE if prog >= 80 else AMAR),
        kpi("Velocidade",     f"{vel}",         "un/min",            AZUL2),
        kpi("Status",         STATUS_LABEL.get(status, status.upper()), "",
            STATUS_COR.get(status, CINZA)),
    ]

    banner = html.Div(
        [html.Strong("⚠ ALARME: "), alarme],
        style={"background": "#fff3cd", "border": f"1px solid {AMAR}",
               "borderRadius": "10px", "padding": "14px 20px",
               "fontWeight": "600", "color": "#856404", "marginBottom": "20px"}
    ) if alarme else html.Div()
    ...
    if hist:
        fig.add_trace(go.Scatter(
            x=[r["ts"] for r in hist], y=[r["valor"] for r in hist],
    ...
    gauge = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            "threshold": {"line": {"color": VERDE, "width": 3},
                          "thickness": 0.75, "value": 100},
        },
    ))
    gauge.update_layout(paper_bgcolor="white",
    ...
    painel_os = html.Div([
        html.H3("Ordens em Andamento",
                style={"margin": "0 0 14px", "fontSize": "15px",
                       "color": AZUL, "fontWeight": "700"}),
        *(os_items or [html.P("Nenhuma OS em andamento.",
